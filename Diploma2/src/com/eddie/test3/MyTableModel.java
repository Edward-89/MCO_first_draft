...
import com.eddie.test3.ControlPanel.Variant;

public class MyTableModel extends AbstractTableModel implements Serializable, Cloneable{
	...
	public MyTableModel() {
		...
		colName = new String[] {"#id", "K1", "K2", "K3", "K4", "K5", "K6", "K7", "K8", "K9", "K10"};
	...
	public void testData(int rowCount){
		dataModel.removeAll(dataModel);
		int id = 1;
		for(int i = 0; i < rowCount; i++){
			Double[] doubly = new Double[getColumnCount()];
			for(int j = 0 ; j < getColumnCount(); j++){
				if(j == 0) doubly[j] = (double) id++;
				else doubly[j] = Math.random();
			}
			addData(doubly);
		}
	}
	
	public void testData2(int rowCount){
		dataModel.removeAll(dataModel);
		int id = 1;
		for(int i = 0; i < rowCount; i++){
			Double[] doubly = new Double[getColumnCount()];
	...
	public Double[] getMax(){
		...
		for(int j = 0; j < max.length; j++){
			...
			for(int k = 0; k < this.getRowCount(); k++){
				...
				if(max[j] < maximum) max[j] = maximum;
			}
		}
		return max;
	}
	
	public void getMaxOperation(){
		Variant set[] = ControlPanel.getVariant();
	...
	public String[] getEnabledColumn(boolean[] bool){
		...
		for(int i = 0 ; i < columnCount; i++) columnName[i] = this.getColumnName(i);
